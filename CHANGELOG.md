...
- `[pending]` kalshi scraper: apply event-level fee overrides (2026-09-25) — `GET /events/fee_changes` (public) lists per-event overrides of the series fee, mostly scheduled to switch at game start. Overrides whose `scheduled_ts` has passed now replace the series `fee_type`/`fee_multiplier` for that event. None active today.
