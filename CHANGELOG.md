...
- `[pending]` arb scanner: implausible-return cap, 'unverified' tier, CI job summary (2026-09-24). A guaranteed return above 15% (return on capital, after fees) is downgraded — in the audits of both repos every such row was two different questions, and verified real arbs were 0-5%. Downgraded rows (cap, settled-one-side, rules-text warning) are now `unverified` instead of `one-sided`: the tab keeps their stakes and return visible under an orange "? Unverified" badge, never "★ Guaranteed". In Actions, arb_scanner writes every guaranteed/unverified row to the run's job summary.
